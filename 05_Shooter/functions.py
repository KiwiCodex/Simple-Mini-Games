...
def load_highscore():
    path = get_file_path("highscore.txt")
    try:
...
def save_highscore(save_score):
    path = get_file_path("highscore.txt")
    try:
        with open(path, "w") as file:
            file.write(str(save_score))
    except Exception as e:
        print(f"Error saving highscore: {e}")

def draw_lives(screen, hp, x, y, heart_image):
    for i in range(hp):
...
# Game Over Screen
def show_game_over(screen: pygame.Surface, clock: pygame.time.Clock, highscore):
    ...
    while waiting:
        ...
        score(screen, c.console_font, f"Current Highscore: {highscore}", c.GREEN, center_x, center_y -20)
